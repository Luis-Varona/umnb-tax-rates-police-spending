...
def main():
    for year in list(range(2000, 2005)) + list(range(2006, 2019)):
        source = f'data_xlsx/{year}/GNB{year}_bgt_exps.xlsx'
        dest = f'data_clean/{year}/GNB{year}_bgt_exps_clean.xlsx'
...
def clean_bgt_exps_data(source: str) -> pl.DataFrame:
    ...
    return (expenditure_df
            ...
            .drop("Index")
            .with_columns((~cs.matches("Municipality|General Government"))
                          .cast(pl.Int64))
            .with_columns(pl.col("General Government")
                          .cast(pl.Float64))
